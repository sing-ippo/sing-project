...
VALID_CATEGORIES = {
    ...
    "ит_системы", "бюрократия", "общежитие", "лекции", "поступление"
}

STATIC_SOURCES = {
    "manual", "dod", "chat_экзамены_мирэа", "chat_поступление_иит", 
    "chat_поступление_иит_2024", "chat_подслушано_мирэа"
}
...
def check_base_fields(entry: dict) -> list[str]: # Валидация Question и Answer
    ...
    if not isinstance(a, str) or not a.strip():
                errors.append("Для чатов 'answer' должен быть None или непустой строкой")
    return errors
...
def check_consistency(entry: dict) -> list[str]: # Кросс-полевая валидация (Consistency Check)
    ...
    elif src == "dod" and cat != "поступление":
        errors.append(f"Для source 'dod' ожидалась категория 'поступление', получено '{cat}'")
    elif eid >= 2000 and not src.startswith("chat_") and src != "dod":
        errors.append(f"Для ID {eid} источник должен начинаться с 'chat_' или быть 'dod', получено '{src}'")
