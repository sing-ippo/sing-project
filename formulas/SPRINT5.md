...
В Sprint 4 ты добавил `/render` endpoint (LaTeX → PNG через matplotlib) и третий предмет (economic). Валидация во всех endpoints уже сделана хорошо (`/render`: пустая строка → 400, слишком длинная → 400, ошибка рендера → 422; `/extract`: проверка content_type и magic bytes PDF). **Её НЕ ТРОГАЙ — менять там нечего.**

Что осталось доделать — три вещи: один баг в `requirements.txt`, упаковка в Docker, тесты.
